...
class newship {
  ...
  AddFn(data) {
    ...
    const DF = this.DefiningFeature(data);
    const coor = [DF.lon, DF.lat];
    const geom = new Point(coor);
    const f = new Feature(Object.assign({ geometry: geom, center: coor }, DF));
  ...
  // 更新船舶
  replaceship(item) {
    const f = this.Layer.getSource().getFeatureById(item[this.id]);
    // console.log(f)
