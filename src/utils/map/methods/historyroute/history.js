...
class addhistorymethods {
  ...
  addhistory(dataList, map, minpx, mimangle, allpoint) {
    ...
    for (let i = 0; i < dataFifterList.length; i++) {
      let coor = transform(
        [
          dataFifterList[i].lon / 60 / 10000,
          dataFifterList[i].lat / 60 / 10000,
        ],
        "EPSG:4326",
        "EPSG:3857"
      );
    ...
    for (let i = 0; i < dataFifterList.length; i++) {
      ...
      var FeaturePoint = new Feature({
        ...
        lonStr: MAP.CacuLonLat(
          dataFifterList[i].lon / 60 / 10000,
          "lon",
          "String"
        ),
        latStr: MAP.CacuLonLat(
          dataFifterList[i].lat / 60 / 10000,
          "lat",
          "String"
        ),
...
function HistroyRoute(
  ...
  //显示轨迹
  const showhistory = new addhistorymethods(
    historypointLayerId,
    historylineLayerId,
    map
  );
