...
export default {
  name: "WharfManagement", //码头管理
  components: { NavTable, FormDetail, Pagination },
  data() {
    return {
      activeName: "CompanyWharfLists",
      searchName: "",
      searchCompany: "",
      companyList: [],
      tabList: [
        { label: "企业停泊位置列表", name: "CompanyWharfLists" },
        { label: "企业停泊位置添加", name: "CompanyWharfAdd" },
      ],
      tableData: [],
      total: 0,
      currentPage: 1,
  ...
  methods: {
    ...
    tableItemEdit(value) {
      this.$refs["NavTable"].activeValue = "CompanyWharfAdd";
      this.tabList[1].label = "企业停泊位置修改";
    ...
    tableItemDelete(value) {
      this.$confirm("确定删除吗?", "提示", {
        ...
        .then(() => {
          deleteData(value.id, 1).then((res) => {
            if (res.code == 200) {
    ...
    changeTabItem(value) {
      ...
      this.tabList[1].label = "企业停泊位置添加";
      this.formStatus = "add";
    },
    navBack() {
      this.$refs["NavTable"].activeValue = "CompanyWharfLists";
      this.getTableList();
      this.tabList[1].label = "企业停泊位置添加";
    },
    search() {
